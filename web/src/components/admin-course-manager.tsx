...
export function AdminCourseManager({ courses: initialCourses }: { courses: AdminCourse[] }) {
  ...
  function siblings(parentSectionId: string | null): OutlineItem[] {
    const childSections = sections.filter((section) => section.course_id === selectedId && section.parent_section_id === parentSectionId).map((section) => ({ ...section, kind: "section" as const }));
    const childLessons = lessons.filter((lesson) => lesson.course_id === selectedId && lesson.section_id === parentSectionId).map((lesson) => ({ ...lesson, kind: "lesson" as const }));
        ...
        <div className="admin-outline-item my-2 flex flex-wrap items-center gap-2 p-3">
          ...
          <form className="flex min-w-0 flex-1 flex-wrap items-center gap-2" onSubmit={(event) => void saveOutlineItem(event, item)}>
            <input
              ...
              className="admin-input min-w-48 flex-1 px-3 py-2 text-sm"
              defaultValue={item.custom_title ?? ""}
              name="custom_title"
              placeholder={item.detected_title}
            />
            <label className="flex items-center gap-1 text-xs">
              <input defaultChecked={item.is_visible} name="is_visible" type="checkbox" />
              Visible
            </label>
            <button className="admin-small-button px-3 py-2 text-xs disabled:opacity-50" disabled={Boolean(busy)} type="submit">
              Guardar
            </button>
          </form>
          <button aria-label="Subir" className="admin-order-button px-2 py-1 disabled:opacity-30" disabled={Boolean(busy) || index === 0} onClick={() => void move(item, -1)} type="button">
            ↑
          </button>
          <button
            aria-label="Bajar"
            className="admin-order-button px-2 py-1 disabled:opacity-30"
            disabled={Boolean(busy) || index === group.length - 1}
        ...
        <div className="flex max-h-[65vh] flex-col gap-1 overflow-y-auto">
          {courses.map((course) => (
            <button
              ...
              className={`admin-course-option px-3 py-3 text-left text-sm ${course.id === selectedId ? "admin-course-option-active" : ""}`}
              key={course.id}
              onClick={() => {
        ...
        <div className="flex min-w-0 flex-col gap-6">
          <form className="admin-panel admin-course-form p-6" key={selected.id} onSubmit={saveCourse}>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Detectado en Drive:{" "}
              <a href={`https://www.google.com/search?q=${selected.detected_title}&tbm=isch`} target="_blank">
                {selected.detected_title}
              </a>
            </p>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-sm">
                Título
                <input className="admin-input px-3 py-2" defaultValue={selected.custom_title ?? ""} name="custom_title" />
              </label>
              <label className="flex flex-col gap-1 text-sm">
                Autor
                <input className="admin-input px-3 py-2" defaultValue={selected.author ?? ""} name="author" />
              </label>
              <label className="flex flex-col gap-1 text-sm">
                Plataforma
                <input className="admin-input px-3 py-2" defaultValue={selected.platform ?? ""} name="platform" />
              </label>
              <label className="flex flex-col gap-1 text-sm">
                Fecha
                <input className="admin-input px-3 py-2" defaultValue={selected.published_on ?? ""} name="published_on" placeholder="AAAA-MM-DD" />
              </label>
              <label className="flex flex-col gap-1 text-sm sm:col-span-2">
                Portada (URL)
                <input className="admin-input px-3 py-2" defaultValue={selected.cover_url ?? ""} name="cover_url" type="url" />
              </label>
              <label className="flex flex-col gap-1 text-sm sm:col-span-2">
                Descripción
                <textarea className="admin-input admin-textarea min-h-28 px-3 py-2" defaultValue={selected.description ?? ""} name="description" />
              </label>
              <label className="flex items-center gap-2 text-sm sm:col-span-2">
                <input defaultChecked={selected.is_visible} name="is_visible" type="checkbox" />
                Visible en el catálogo
              </label>
            </div>
            <button className="primary-button mt-6 disabled:opacity-60" disabled={Boolean(busy)} type="submit">
              {busy === `course:${selected.id}` ? "Guardando…" : "Guardar curso"}
            </button>
          </form>
          ...
          <section className="admin-panel admin-outline-panel p-6">
            ...
            <div className="mt-4">
              {outlineLoading ? <p className="text-sm text-slate-400">Cargando contenido del curso…</p> : outlineError ? <p className="text-rose-200">{outlineError}</p> : renderGroup(null)}
            </div>
          </section>
