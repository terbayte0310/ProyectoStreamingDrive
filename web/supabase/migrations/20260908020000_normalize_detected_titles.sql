-- Separate the raw Drive name (detected_name, kept as-is for identity/audit) from a
-- normalized display title (detected_title) so the catalog stops showing numeric
-- prefixes, underscores and hyphens straight from Drive filenames. Drive itself is
-- never touched: normalization only changes what gets written into Supabase.
...
begin
  ...
  select p_source_id, drive_item.id, snapshot.detected_title, snapshot.detected_position
  from sync_snapshot snapshot
  ...
  select category.id, course_item.id, snapshot.detected_title, snapshot.detected_position
  from sync_snapshot snapshot
  ...
  select course.id, section_item.id, null, snapshot.detected_title, snapshot.detected_position
  from sync_snapshot snapshot
